...
function AdminContent() {
  ...
  const [content, setContent] = useState({
    hero: { title: 'Professional HDPE Plastic Netting Manufacturer Since 2005', subtitle: 'Leading manufacturer of construction safety nets, agricultural nets, and specialty plastic netting solutions.', cta1: 'Get Free Quote', cta2: 'View Products' },
    about: { title: 'About JIACHENG NETTING', subtitle: 'Your Trusted HDPE Netting Partner Since 2005', description: 'JIACHENG NETTING is a leading manufacturer of high-quality HDPE plastic netting products.', features: 'BSCI Certified, ISO14001 Certified, 20,000m² Production, Export to 50+ Countries' },
    contact: { email: 'sales@jiachengnetting.com', phone: '+86 531 8888 8888', whatsapp: '+86 138 0000 0000', address: 'No. 88 Industrial Park Road, Jinan City, Shandong Province, China', workingHours: 'Monday - Saturday: 8:00 AM - 6:00 PM (CST)' },
    footer: { company: 'JIACHENG NETTING', copyright: '© 2024 JIACHENG NETTING. All Rights Reserved.' }
  });
  ...
  if (!isAuthenticated) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: '#f5f7fa', fontFamily: 'system-ui, sans-serif' }}>
        <div style={{ backgroundColor: 'white', padding: '40px', borderRadius: '16px', boxShadow: '0 4px 20px rgba(0,0,0,0.1)', width: '100%', maxWidth: '400px', textAlign: 'center' }}>
          ...
          <button onClick={handleLogin} style={{ width: '100%', padding: '14px', backgroundColor: '#1e40af', color: 'white', border: 'none', borderRadius: '10px', fontSize: '16px', fontWeight: '600', cursor: 'pointer' }}>
            登录
          </button>
        </div>
      </div>
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#f5f7fa', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ display: 'flex' }}>
        <aside style={{ width: '240px', backgroundColor: 'white', borderRight: '1px solid #e0e0e0', padding: '20px 0', minHeight: 'calc(100vh - 80px)' }}>
          {tabs.map((tab) => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{ display: 'block', width: '100%', padding: '16px 24px', textAlign: 'left', border: 'none', backgroundColor: activeTab === tab.id ? '#eff6ff' : 'transparent', color: activeTab === tab.id ? '#2563eb' : '#374151', fontWeight: activeTab === tab.id ? '600' : '400', fontSize: '15px', cursor: 'pointer', borderLeft: activeTab === tab.id ? '4px solid #3b82f6' : '4px solid transparent' }}>
              {tab.label}
            </button>
          ))}
        </aside>
